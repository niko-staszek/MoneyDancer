...
void EnsureSeriesActive(int dir)
{
   if(dir > 0)
   {
      if(!g_buySeriesActive)
      {
         ...
         g_buySeriesOpenEquity = AccountInfoDouble(ACCOUNT_EQUITY);   // S1.0
         g_buySeriesSLFired    = false;                                // S1.0
      }
   }
   else
   {
      if(!g_sellSeriesActive)
      {
         ...
         g_sellSeriesOpenEquity = AccountInfoDouble(ACCOUNT_EQUITY);  // S1.0
         g_sellSeriesSLFired    = false;                               // S1.0
      }
   }
}
