...
bool CheckShouldActivateE(int basketDir, string &reason)
{
   ...
   // S2.A.7 Option 3 — MMD-aware adverse gate. When enabled, ScenarioE only
   // activates if MMD currently opposes the basket direction. Prevents hedge
   // runners from firing on random basket DDs during range regimes.
   if(UseMMDAdverseGateForE && UseMMDClassifier)
   {
      int mmd = MMD_RegimeSimple();
      if(mmd == 0 || mmd == basketDir)
      {
         // Not in adverse MMD — don't activate even if other triggers fire.
         reason = "";
         return false;
      }
   }
