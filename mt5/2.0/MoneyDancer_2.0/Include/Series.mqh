...
//| increment the counter and mark active. S1.0: snapshot equity at  |
//| series open and clear the per-series SL-fired flag.               |
//+------------------------------------------------------------------+
void EnsureSeriesActive(int dir)
{
   if(dir > 0)
   {
      ...
      {
         g_buySeriesId++;
         g_buySeriesActive     = true;
         g_buySeriesOpenEquity = AccountInfoDouble(ACCOUNT_EQUITY);
         g_buySeriesSLFired    = false;
      }
   }
   else
   {
      ...
      {
         g_sellSeriesId++;
         g_sellSeriesActive     = true;
         g_sellSeriesOpenEquity = AccountInfoDouble(ACCOUNT_EQUITY);
         g_sellSeriesSLFired    = false;
      }
   }
}
