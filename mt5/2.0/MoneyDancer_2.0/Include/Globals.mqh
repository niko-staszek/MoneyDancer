...
// S1.0 — per-series snapshot of account equity at the moment the series began.
// Used as the denominator for MaxBasketLossPct. Captured in EnsureSeriesActive().
double g_buySeriesOpenEquity  = 0.0;
double g_sellSeriesOpenEquity = 0.0;

// S1.0 — per-series "SL fired" flag. While set, the series stays inactive and
// Scenario E does not promote any of its positions to runners. Cleared when the
// next series opens.
bool g_buySeriesSLFired  = false;
bool g_sellSeriesSLFired = false;

// S1.0 — daily basket-SL trigger counter. Reset on day change (Risk.mqh).
int g_basketSLToday  = 0;
