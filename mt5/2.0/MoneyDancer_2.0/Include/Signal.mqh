...
bool EvaluateSecond(int &dirOut, int &peakTicksOut, int &movePtsOut)
{
   ...
   if(movePts < EffectiveMinMovePoints()) return false;
...
bool SendOrder(int dir, double lots, bool useTP, int tpPoints, bool isRunner, string commentText)
{
   ...
   double baseLot = ComputeBaseLot();   // S1.5 — equity-scaled when LotsBasePerThousand>0
   if(wantPyr)   lots = baseLot;        // pyramid always uses basic lot
   if(lots <= 0) lots = baseLot;
   lots = ClampLot(lots);
...
void HandleSignal(int signalDir)
{
   ...
   // S2.A.2 — skip-trade gate when ATR is below floor (low-vol cells with no edge).
   if(ATRGateBlocks()) return;

   // S3.2 — block new grid entries when the regime classifier flags a trend.
   // S3.2b: when RegimeTrendMode=WITH_TREND and MMD is on, only opposite-of-trend
   //        entries are blocked. Pyramid is unaffected either way.
   if(RegimeBlocksEntryDir(signalDir)) return;

   // S1.1 + S1.4 + S2.0 — entry-side blockers (news blackout / spread spike /
   // hour blocklist). All default OFF; opt-in via the corresponding inputs.
   if(Guards_BlocksEntry()) return;
   ...
   // S2.A.7 Option 2 — block ScenarioD adds when MMD opposes basket dir,
   // even when ScenarioE is off. Strictly opt-in via BlockDOnAdverseMMD.
   if(BlockDOnAdverseMMD && UseMMDClassifier)
   {
      int mmd_d = MMD_RegimeSimple();
      if(mmd_d != 0 && mmd_d != basketDir) return;
   }
   ...
   {
      // S5.5a fix: price is recovering toward BE — use the basket's original
      // first-lot, not ComputeBaseLot(). The recomputed base shrinks if equity
      // dropped due to floating loss, weakening the recovery basket BE pull.
      // FirstBasketLotSeries preserves the lot the basket was sized at when
      // it opened, keeping recovery adds proportional to the basket's geometry.
      double firstLotDB = FirstBasketLotSeries(basketDir, seriesCmt);
      if(firstLotDB <= 0.0) firstLotDB = ComputeBaseLot();  // fallback
      lot  = ClampLot(firstLotDB);
      cmtD = seriesCmt + "|DB";
   }
