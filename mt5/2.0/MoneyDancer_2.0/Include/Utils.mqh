...
//+------------------------------------------------------------------+
//| S1.5 - effective base lot for new entries.                       |
//| When LotsBasePerThousand > 0: lot = (equity / 1000) * scale, then |
//| clamped to broker min/step. Else: legacy fixed LotsBase.          |
//+------------------------------------------------------------------+
// Forward declaration — defined in MMD.mqh (included later in load order).
int MMD_RegimeSimple();

double ComputeBaseLot()
{
   double base;
   if(LotsBasePerThousand <= 0.0)
   {
      base = LotsBase;
   }
   else
   {
      ...
      base = (eq <= 0.0) ? LotsBase : (eq / 1000.0) * LotsBasePerThousand;
   }

   // S2.A.3 — regime-aware multiplier. Skip when both are 1.0 (legacy) or MMD off.
   if(UseMMDClassifier && (LotMultRange != 1.0 || LotMultTrend != 1.0))
   {
      int reg = MMD_RegimeSimple();
      double mult = (reg == 0) ? LotMultRange : LotMultTrend;
      if(mult > 0.0 && mult != 1.0) base *= mult;
   }

   return ClampLot(base);
}

//+------------------------------------------------------------------+
//| S2.A — adaptive MinMovePoints. Caches one ATR read per bar of the |
//| ATR timeframe. INVERSE: high vol -> lower threshold (let strategy |
//| grind small bursts). LINEAR: high vol -> higher threshold.         |
//+------------------------------------------------------------------+
int      g_minMoveAtrHandle    = INVALID_HANDLE;
double   g_minMoveAtrLast      = 0.0;
datetime g_minMoveAtrBar       = 0;
bool     g_minMoveAtrInitFailed = false;
int      g_minMoveCached       = 0;

//+------------------------------------------------------------------+
//| S2.A.2 — current ATR in points. Shared cache with EffectiveMinMove. |
//| Returns 0 if ATR handle not initialized (caller treats as "unknown"). |
//+------------------------------------------------------------------+
double CurrentATRPoints()
{
   // Reuse cache populated by EffectiveMinMovePoints (g_minMoveAtrLast). If
   // EffectiveMinMovePoints hasn't been called yet, init the handle here.
   if(g_minMoveAtrInitFailed) return 0.0;
   if(g_minMoveAtrHandle == INVALID_HANDLE)
   {
      g_minMoveAtrHandle = iATR(_Symbol, MinMoveATRTimeframe, MinMoveATRPeriod);
      if(g_minMoveAtrHandle == INVALID_HANDLE)
      {
         g_minMoveAtrInitFailed = true;
         return 0.0;
      }
   }
   datetime bar = iTime(_Symbol, MinMoveATRTimeframe, 0);
   if(bar == g_minMoveAtrBar && g_minMoveAtrLast > 0.0) return g_minMoveAtrLast;

   double buf[];
   ArraySetAsSeries(buf, true);
   if(CopyBuffer(g_minMoveAtrHandle, 0, 0, 1, buf) <= 0) return g_minMoveAtrLast;
   double atr_price = buf[0];
   if(atr_price <= 0.0) return g_minMoveAtrLast;
   g_minMoveAtrLast = atr_price / _Point;
   g_minMoveAtrBar  = bar;
   return g_minMoveAtrLast;
}

bool ATRGateBlocks()
{
   if(MinATRPointsForEntry <= 0) return false;
   double atr_pts = CurrentATRPoints();
   if(atr_pts <= 0.0) return false;  // unknown ATR -> don't block
   return atr_pts < (double)MinATRPointsForEntry;
}

int EffectiveMinMovePoints()
{
   if(MinMoveAdaptiveMode == MM_FIXED) return MinMovePoints;

   // Lazy init of the ATR handle so custom-symbol tester runs can attach the
   // indicator after ticks are ingested.
   if(g_minMoveAtrInitFailed) return MinMovePoints;
   if(g_minMoveAtrHandle == INVALID_HANDLE)
   {
      g_minMoveAtrHandle = iATR(_Symbol, MinMoveATRTimeframe, MinMoveATRPeriod);
      if(g_minMoveAtrHandle == INVALID_HANDLE)
      {
         g_minMoveAtrInitFailed = true;
         PrintFormat("[S2.A] iATR init failed (err=%d) on %s tf=%d period=%d - MinMove adaptive disabled",
                     GetLastError(), _Symbol, (int)MinMoveATRTimeframe, MinMoveATRPeriod);
         return MinMovePoints;
      }
   }

   datetime bar = iTime(_Symbol, MinMoveATRTimeframe, 0);
   if(bar == g_minMoveAtrBar && g_minMoveCached > 0) return g_minMoveCached;

   double buf[];
   ArraySetAsSeries(buf, true);
   if(CopyBuffer(g_minMoveAtrHandle, 0, 0, 1, buf) <= 0)
      return (g_minMoveCached > 0 ? g_minMoveCached : MinMovePoints);

   double atr_price = buf[0];  // in price units (e.g., 0.40 for 40 pts on XAUUSD)
   if(atr_price <= 0.0) return MinMovePoints;
   double atr_pts = atr_price / _Point;  // convert to points

   double result = 0.0;
   if(MinMoveAdaptiveMode == MM_ATR_INVERSE)
      result = MinMoveATRConstant / atr_pts;
   else if(MinMoveAdaptiveMode == MM_ATR_LINEAR)
      result = MinMoveATRMult * atr_pts;
   else
      return MinMovePoints;

   int rounded = (int)MathRound(result);
   if(rounded < MinMovePointsMin) rounded = MinMovePointsMin;
   if(rounded > MinMovePointsMax) rounded = MinMovePointsMax;

   g_minMoveAtrLast = atr_pts;
   g_minMoveAtrBar  = bar;
   g_minMoveCached  = rounded;
   return rounded;
}
