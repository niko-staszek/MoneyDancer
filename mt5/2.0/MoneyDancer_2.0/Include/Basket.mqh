...
//+------------------------------------------------------------------+
//| S1.0 — per-basket equity SL check (read-only).                   |
//| Compares basket floating P&L (excluding runners) against the     |
//| equity snapshot taken when the series opened. Returns true when  |
//| the loss meets or exceeds MaxBasketLossPct%. Risk.mqh does the   |
//| actual closing + state mutation.                                  |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| S2.A.7 Option 1 — regime-direction-aware basket SL threshold.    |
//| Returns the effective MaxBasketLossPct for the given basket dir,  |
//| based on current MMD regime.                                      |
//|                                                                   |
//| When all Range/TrendWith/TrendAgainst inputs are 0 OR             |
//| UseMMDClassifier=false, returns MaxBasketLossPct (legacy single   |
//| threshold). Otherwise picks the appropriate per-regime threshold. |
//+------------------------------------------------------------------+
double EffectiveBasketLossPct(int basketDir)
{
   // Legacy mode: all override inputs are 0 or MMD off
   bool any_override = (MaxBasketLossPctRange > 0.0
                     || MaxBasketLossPctTrendWith > 0.0
                     || MaxBasketLossPctTrendAgainst > 0.0);
   if(!any_override || !UseMMDClassifier) return MaxBasketLossPct;

   int mmd = MMD_RegimeSimple();
   double pick = MaxBasketLossPct;

   if(mmd == 0)
   {
      if(MaxBasketLossPctRange > 0.0) pick = MaxBasketLossPctRange;
   }
   else if(mmd == basketDir)  // MMD aligns with basket — winning trend
   {
      if(MaxBasketLossPctTrendWith > 0.0) pick = MaxBasketLossPctTrendWith;
   }
   else  // MMD opposes basket — losing trend
   {
      if(MaxBasketLossPctTrendAgainst > 0.0) pick = MaxBasketLossPctTrendAgainst;
   }

   return pick;
}

bool BasketEquitySLTriggered(int dir)
{
   double threshold = EffectiveBasketLossPct(dir);
   if(threshold <= 0.0)          return false;
   if(!SeriesActive(dir))        return false;

   double openEq = (dir > 0 ? g_buySeriesOpenEquity : g_sellSeriesOpenEquity);
   if(openEq <= 0.0) return false;

   double pl = BasketFloatingPL(dir, false);
   if(pl >= 0.0) return false;

   double lossPct = (-pl) / openEq * 100.0;
   return (lossPct >= threshold);
}

// Strong counter-trend blocks further ScenarioD escalation.
// S2.A.7 Option 2: also block when MMD opposes basket direction (BlockDOnAdverseMMD).
bool TrendBlocksD(int basketDir)
{
   // Slope-based gate (existing logic)
   if(UseSlopeFilter)
   {
   ...
   // S2.A.7 Option 2 — MMD-based adverse gate
   if(BlockDOnAdverseMMD && UseMMDClassifier)
   {
      int mmd = MMD_RegimeSimple();
      if(mmd != 0 && mmd != basketDir) return true;  // MMD opposes basket
   }
