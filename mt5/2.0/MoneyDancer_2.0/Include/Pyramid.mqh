...
void PyramidManage()
{
   // S3.2c: in fixed-TP mode, positions self-manage via the per-position TP set
   // at open time. PyramidManage skips BUILDING/COASTING overrides entirely;
   // BE management is also skipped since each position has its own exit.
   if(PyramidFixedTPPts > 0 && PyramRange > 0)
   {
      // Clean up tickets that have already exited via their fixed TP.
      for(int i = PyrCount() - 1; i >= 0; i--)
      {
         if(!PyrTicketStillActive(g_pyrTickets[i]))
            PyrRemoveAt(i);
      }
      return;
   }
...
void PyramidOnNewPosition(ulong ticket)
{
   ...
   // S3.2c: when PyramidFixedTPPts > 0, set a per-position TP at entry instead
   // of TP=0 (legacy BUILDING). PyramidManage will skip TP overrides for these.
   if(PyramidFixedTPPts > 0)
   {
      double fixedTP = (dir > 0 ? op + PyramidFixedTPPts * _Point
                                : op - PyramidFixedTPPts * _Point);
      fixedTP = NormalizePrice(fixedTP);
      ModifyPositionSLTP(ticket, PositionGetDouble(POSITION_SL), fixedTP);
   }
   else
   {
