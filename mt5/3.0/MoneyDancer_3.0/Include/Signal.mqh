//+------------------------------------------------------------------+
//| Signal.mqh â€” tick-burst detection + cooldowns + order orchestrator|
//| Phase A5.6                                                       |
//|                                                                   |
//| Contains:                                                         |
...
//|                                                                   |
//| MT5 adaptations:                                                  |
//|   - Bid/Ask constants â†’ SymbolInfoDouble(_Symbol, SYMBOL_BID/ASK) |
//|   - OrderSend() â†’ CTrade via OpenPosition() (returns ulong ticket)|
//|   - IsTradeAllowed() â†’ TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)|
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| SendOrder â€” routing-aware entry. Delegates to OpenPosition() for |
...
bool SendOrder(int dir, double lots, bool useTP, int tpPoints, bool isRunner, string commentText)
{
   ...
   // Pyramid routing (doesn't change the signal â€” decides at open-time).
   bool wantPyr = false;
...
//+------------------------------------------------------------------+
//| HandleSignal â€” main dispatcher. Routes to basic / martingale /   |
...
void HandleSignal(int signalDir)
{
   ...
   // â€” block new grid entries when ADX flags a strong trend.
   if(RegimeBlocksGridEntries()) return;
   ...
   {
      g_scenarioStatus = "E";
      // Don't keep adding to the losing basket â€” open a runner instead.
...
//+------------------------------------------------------------------+
//| DetectAndHandleSignal â€” top-level per-tick entry. Chooses SECOND |
