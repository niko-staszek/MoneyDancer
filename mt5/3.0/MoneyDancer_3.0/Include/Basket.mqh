//+------------------------------------------------------------------+
//| Basket.mqh â€” basket math: P&L, lot sums, BE (with or without     |
//|             costs), step gates, price-vs-BE, min-distance,       |
//|             equity/basket/trend guards.                          |
//| Phase A5.3                                                       |
//|                                                                   |
//| No CTrade calls here â€” this module is pure READ (analytics).     |
//| Actions (setting TP, opening runners, closing baskets) belong    |
//| in ScenarioD / ScenarioE / Risk.                                  |
//|                                                                   |
//| MT5 semantic adaptations vs MT4:                                  |
//|  - OrdersTotal + OrderSelect(MODE_TRADES) â†’ PositionsTotal +     |
//|    PositionGetTicket + PositionGetXxx.                            |
//|  - OrderProfit+OrderSwap+OrderCommission â†’ POSITION_PROFIT +     |
//|    POSITION_SWAP. Commission in MT5 is on the entry DEAL, not    |
//|    the position â€” for a live basket BE calculation, the impact   |
...
//+------------------------------------------------------------------+
//| Step gates â€” block new basket adds until price has moved far     |
...
//+------------------------------------------------------------------+
//| Price vs. BE helpers â€” ScenarioD uses these to avoid unsafe adds.|
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Guards â€” used by Scenario E activation and Risk module.          |
//+------------------------------------------------------------------+
...
// â€” read-only equity SL check against the series-open equity snapshot.
bool BasketEquitySLTriggered(int dir)
{
   if(MaxBasketLossPct <= 0.0) return false;
   ...
   return (lossPct >= MaxBasketLossPct);
}
