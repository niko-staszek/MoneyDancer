//+------------------------------------------------------------------+
//| Dashboard.mqh â€” UI layer (A7b full port)                         |
//|                                                                   |
//| Port of MT4 legacy dashboard: Wall Street panel + trade markers  |
//| + bottom M15 PnL strip + control buttons + AI-sim status. All    |
//| visual â€” strategy has zero functional dependency on anything     |
//| here.                                                             |
//|                                                                   |
//| MT4â†’MT5 semantic adaptations applied inline:                      |
//|   * ObjectCreate/ObjectSet already MT5-style in MT4 source (chart|
//|     id passed as first arg) â€” no translation needed for draws.   |
//|   * Open-position iteration: OrdersTotal+OrderSelect(MODE_TRADES)|
//|     â†’ PositionsTotal+PositionGetTicket+PositionSelectByTicket.    |
//|   * History iteration: OrdersHistoryTotal+OrderSelect(MODE_HISTORY|
//|     â†’ HistorySelect(from,to) + HistoryDealsTotal +                |
//|     HistoryDealGetTicket/HistoryDealGet*. We only process         |
//|     DEAL_ENTRY_OUT deals (closes), matching the MT4 "closed      |
//|     trade" semantic.                                              |
//|   * OP_BUY/OP_SELL â†’ POSITION_TYPE_BUY/SELL (open) or direction   |
//|     sign derived from DEAL_TYPE for closed deals.                 |
//|   * Bid/Ask â†’ SymbolInfoDouble(_Symbol, SYMBOL_BID/ASK).          |
//|   * Point â†’ _Point. AccountEquity() â†’ AccountInfoDouble(...).    |
//|   * OrderClose() â†’ CTrade::PositionClose(ticket) via g_dashTrade. |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| HISTORY ITERATION HELPERS (MT4â†’MT5 adaptation)                    |
//|                                                                   |
...
void CollectClosedDealForBasket(ulong outDealTicket)
{
   ...
   datetime openT = 0;
   double openP = closeP;  // fallback â€” degenerate line if IN lookup fails
...
void DrawBottomResultsPanel()
{
   ...
   // Throttle to once per second â€” M15 bucket data only changes per-minute at
   ...
   // text/colour refreshed in place by CreateBottomLabel below â€” so there's
   // no deleteâ†’recreate flicker in the common case.
...
//| handles visual state only â€” bucketing + marker collection +       |
...
//| OBJPROP_STATE=true on click â€” we reset to false after handling.   |
//+------------------------------------------------------------------+
