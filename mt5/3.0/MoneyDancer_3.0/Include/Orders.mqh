//+------------------------------------------------------------------+
//| Orders.mqh â€” trade primitives built on CTrade                    |
...
//|  - OrderSend() â†’ trade.PositionOpen()                             |
//|  - OrderModify() â†’ trade.PositionModify()                         |
//|  - OrderClose() â†’ trade.PositionClose()                           |
//|  - Iteration: OrdersTotal+OrderSelect(MODE_TRADES) â†’ PositionsTotal+PositionGetTicket
//|  - OrderProfit+OrderSwap+OrderCommission â†’ POSITION_PROFIT+POSITION_SWAP  |
...
//|    is charged on close â€” PROFIT+SWAP is the correct "running P&L")    |
//|  - MODE_STOPLEVEL â†’ SYMBOL_TRADE_STOPS_LEVEL                      |
//+------------------------------------------------------------------+
#ifndef __MD_ORDERS_MQH__
#define __MD_ORDERS_MQH__

//+------------------------------------------------------------------+
//| Global CTrade instance â€” used by all modules.                    |
...
//| markers, and timing globals â€” this is a thin primitive.          |
//+------------------------------------------------------------------+
...
bool ModifyPositionSLTP(ulong ticket, double newSL, double newTP)
{
   ...
   // Stop-level guard â€” MT5 equivalent of MT4 MODE_STOPLEVEL
...
//| deals, not live positions â€” see header comment).                  |
//+------------------------------------------------------------------+
