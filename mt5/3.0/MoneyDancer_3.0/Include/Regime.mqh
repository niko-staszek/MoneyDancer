//+------------------------------------------------------------------+
//| Regime.mqh â€” ADX regime gate (1.2-line: ADX only).          |
//|                                                                    |
//| When RegimeMode = HARD and ADX >= RegimeAdxThresh, HandleSignal   |
//| short-circuits â€” no new grid entries. Pyramid is unaffected (its  |
...
bool RegimeInit()
{
   // Lazy-init the iADX handle in GetCurrentADX â€” custom symbols in tester
...
double GetCurrentADX()
{
   ...
   if(g_adxHandle == INVALID_HANDLE)
   {
      ...
      if(g_adxHandle == INVALID_HANDLE)
      {
         ...
         PrintFormat("[] iADX init failed (err=%d) on %s tf=%d period=%d - regime gate disabled",
