...
void UpdateDailyBaselineAndMetrics()
{
   ...
   if(dk != g_baseDayKey)
   {
      ...
      // S1.0 — reset basket-SL day counter
      g_basketSLToday  = 0;
      g_basketSLDayKey = dk;
...
//+------------------------------------------------------------------+
//| S1.0 — per-basket equity SL enforcement (with bugfix backport).  |
//+------------------------------------------------------------------+
...
void EnforceBasketSL_Dir(int dir)
{
   ...
   if(closed == 0)
   {
      ...
      if(forced == 0)
      {
         PrintFormat("[S1.0] CRITICAL cannot close any positions; eq=%.2f peak=%.2f",
...
void EnforceBasketSL()
{
   ...
   // BUGFIX 2026-05-17: do NOT respect IsAutoPaused() — open positions still
